...
function OperatorLanding() {
  ...
  if (!routeParams.shopId && viewer?.adminUIShops?.length > 0) {
    return <Redirect to={`/${viewer.adminUIShops[0]._id}`} />;
  } else if (!routeParams.shopId && viewer) {
    return <Redirect to={"/new-shop"} />;
  }
  ...
  return (
    <Fragment>
      ...
      <Grid container direction="column" justify="center" alignItems="center" spacing={5}>
        ...
        <Grid item>
          <Typography align="center" variant="body1">
            See our <MuiLink href="https://docs.reactioncommerce.com/docs/dashboard">Store Operator’s Guide</MuiLink>{" "}
